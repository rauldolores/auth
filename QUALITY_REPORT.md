...
registration/ownership lifecycle only, not a full UX sweep)
...
| HIGH | INT-KEY-001 | Application API key (`kapp_...`, used by `/api/applications/sync`) has no rotation, no revocation, no expiration, no "last used" tracking, and zero admin-panel UI — the only recovery from a leaked key is deleting and re-registering the whole application | Integration Surface | VERIFIED (2026-08-11, commit 585b646) — migration 0032 adds rotate/revoke/last-used tracking plus a real admin-panel UI; also found and closed in the same pass: `api_key_hash` was readable by any authenticated user (table-level grant made the later per-column revoke a no-op) — now column-ACL restricted to `service_role`. Live-verified end to end: rotated Faqturia's real key, confirmed the new key works and the old one doesn't, both events landed in `audit_logs` |
...
| 2026-08-11 (GENERATE/HARDEN, commit 585b646) | kontrolia-integration-surface | 3 gaps closed | — | User selected INT-KEY-001 (HIGH) + INT-KEY-002 + INT-SEC-001 (both MEDIUM) from the audit above to address, explicitly scoped, per this skill's own "AUDIT never chains into fixing" boundary. Migration 0032 added application API key rotation/revocation/`api_key_last_used_at`, closing INT-KEY-001; found and closed in the same pass, not separately tracked: `api_key_hash` was readable by any authenticated user (table-level grant made a per-column revoke a no-op) — now column-ACL restricted to `service_role`. `logSecurityEvent()` added on every auth-rejection branch across `/api/applications/sync` and `/api/oauth-clients`, closing INT-KEY-002. Dependency-free in-memory sliding-window rate limiter added to both routes, closing INT-SEC-001. Live-verified end to end: rotated the real Faqturia application's key, confirmed the new key works and the old one doesn't, confirmed both events landed in `audit_logs`. 149 tests passing across the monorepo at ship time. |
| 2026-08-11T21:23:00 (GENERATE) | kontrolia-integration-surface | 1 new capability delivered | — | User asked to use this skill to build an API for external users; scope disambiguated via AskUserQuestion (member management via the existing `kapp_` application key, not a new key system, not documentation-only) before any code was written, honoring this skill's discover-first discipline. Read `applications/sync/route.ts`, `organization-members/route.ts`, `invitations/route.ts`, `invitations/accept/route.ts`, `packages/db/src/api-key.ts`, and migrations 0025-0031 (the full last-owner-protection chain) before designing anything, specifically to determine whether the protection triggers hold under a `service_role`-authenticated caller — found they don't for owner-*grants* (`auth.role() = 'service_role'` bypass, needed for org bootstrap) but do for owner-*removal*, which shaped the whole design: owner-grant rejection is enforced in application code (`loadAssignableRole()`), owner-removal is left to the database. Built `GET/POST /api/applications/members` (list, invite) and `DELETE/PATCH /api/applications/members/{membershipId}` (remove, grant/revoke roles), all tenant-scoped to the calling application's own organization. While designing the invite path, found and closed an incidental, previously-untracked CRITICAL-class gap: `POST /api/invitations/accept` grants an invitation's `role_id` via a `service_role` client with no owner-role check at all — any org Admin could already create an owner-role invitation and have it silently grant real Owner on acceptance, a reachable bypass of every owner-grant protection built in migrations 0025-0031 through a channel none of them touch. Closed at the source via migration 0033: an invitation can never reference the Owner role, for any caller. Refactored `applications/sync/route.ts` and `organization-members/route.ts` to share the new `application-auth.ts`/`resolve-emails.ts` helpers rather than duplicating the key-verification/email-resolution logic a third time, per this skill's no-duplication principle — confirmed behavior-identical via the full pre-existing test suite passing unchanged. 21 new tests added (auth gate, tenant isolation via cross-org 404, owner-role-grant rejection, last-owner-protection error surfacing, happy paths); 120 tests passing in `apps/auth-server` alone, 12/12 workspace test tasks passing monorepo-wide. Live-verified end to end against the local sandbox: listed real members, created a real invitation, confirmed the owner-role invite is rejected by both the application layer (403) and the database trigger (direct SQL attempt), confirmed cross-org delete returns 404 (not a data leak), confirmed the database blocks revoking the organization's sole active Owner, confirmed granting Owner via role-management is rejected (403), confirmed a legitimate grant/revoke round-trip, and confirmed every mutation left a real trail in `kontrolia_auth.audit_logs` via the pre-existing DB triggers with zero new logging code needed for that. New gap found and left open, not expanded into scope: the new API has no documentation page yet (`INT-DOC-005`, LOW) — deliberately deferred to a separate DOCUMENT-mode pass. |
| 2026-08-11T21:23:00 (AUDIT, scoped) | kontrolia-integration-surface | 2 new HIGH gaps found | — | User asked (after noticing admin-panel's applications page only has Rotar/Revocar, never Crear) whether there's really no page to create an application API key, and whether that belonged in admin-panel. Scoped this audit to the application registration/ownership lifecycle rather than re-running the full-app audit. Read `packages/cli/src/steps/application.ts` (the CLI's own comment: "kontrolia_auth.applications/permissions have no write policy for regular users... and no admin API exists yet for it"), `packages/db/src/register-application.ts` (its INSERT statement genuinely omits `owner_organization_id` — not an oversight in my reading, it's simply never passed), and every RLS policy ever written on `kontrolia_auth.applications` across all 33 migrations (0010's SELECT-only policy, 0018 widening SELECT to the whole catalog, 0022's UPDATE policy — which itself requires `owner_organization_id is not null`, so it can never fire for a freshly-registered application either — and 0032's column-ACL grants). Confirmed via exhaustive grep across `apps/` and `packages/db/src` that zero code anywhere ever writes `owner_organization_id`. Root-caused two compounding gaps: (1) the CLI wizard runs before any organization necessarily exists and never asks which org should own the application it's registering (`askApplicationStep()` takes no org parameter at all); (2) there is also no separate "claim ownership" flow an org could use afterward — no INSERT or ownership-specific UPDATE policy exists for it. Net effect: `INT-KEY-001`'s rotate/revoke UI (shipped and VERIFIED earlier today) and this session's new `/api/applications/members` API are both real and correctly built, but neither is reachable for any application in a genuinely fresh self-hosted deployment without a DBA manually running `UPDATE kontrolia_auth.applications SET owner_organization_id = ...` by hand — which is exactly how the sandbox data used to build and verify both features today was set up, a fact not caught as a gap until this scoped re-audit. Recorded as `INT-API-007` (no self-service creation/ownership path) and `INT-API-008` (the column itself is orphaned) — two sides of one root cause, not independent gaps. Per this skill's own AUDIT-mode boundary, stopped at the finding — no code modified, no fix chosen without asking. |
