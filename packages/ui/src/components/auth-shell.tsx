...
export interface AuthShellProps {
  ...
  subtitle?: string;
  className?: string;
  /** Replaces the default "K" mark + wordmark when set (instance-level custom branding). */
  logoUrl?: string | null;
 ...
 */
export function AuthShell({ title, subtitle, className, logoUrl, children }: AuthShellProps) {
  return (
    <div className="k-flex k-min-h-screen k-items-center k-justify-center k-bg-background k-px-4 k-py-10">
      <div className="k-w-full k-max-w-sm">
        <div className="k-mb-6 k-flex k-items-center k-gap-2.5">
          {logoUrl ? (
            <img src={logoUrl} alt="" className="k-h-9 k-max-w-[200px] k-object-contain" />
          ) : (
            <>
