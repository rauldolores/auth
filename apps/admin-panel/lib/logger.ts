 ...
 *
 * admin-panel has no API routes of its own (all mutations go through
 * auth-server's API or directly to Supabase via RLS from the browser) — this
 * exists for instrumentation.ts's onRequestError hook, the only server-side
 * surface this app actually has.
 */
